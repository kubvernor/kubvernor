...
use crate::backends::envoy::common::{
    HTTPEffectiveRoutingRule, INFERENCE_EXT_PROC_FILTER_NAME, converters, envoy_route_name,
    get_inference_extension_configurations, inference_cluster_name,
};
...
impl From<HTTPEffectiveRoutingRule> for EnvoyRoute {
    fn from(effective_routing_rule: HTTPEffectiveRoutingRule) -> Self {
        ...
        let path_specifier =
            if path_specifier.is_none() { Some(PathSpecifier::Path("/".to_owned())) } else { path_specifier };
        ...
        let service_cluster_names: Vec<_> = super::create_cluster_weights(
            effective_routing_rule.backends.iter().filter_map(|b| match b.backend_type() {
                crate::common::BackendType::Service(service_type_config)
                | crate::common::BackendType::Invalid(service_type_config) => Some(service_type_config),
                crate::common::BackendType::InferencePool(_) => None,
            }),
        );

        let inference_cluster_names: Vec<_> = super::create_cluster_weights(
            effective_routing_rule.backends.iter().filter_map(|b| match b.backend_type() {
        ...
        let action: Action = if let Some(redirect_action) =
            effective_routing_rule.redirect_filter.clone().map(|f| RedirectAction {
