...
impl InferencePoolController {
    ...
    fn error_policy<T>(_object: Arc<T>, err: &ControllerError, _ctx: Arc<InferencePoolControllerContext>) -> Action {
        match err {
...
struct InferencePoolResourceHandler<R> {
    ...
    gateway_patcher: mpsc::Sender<Operation<InferencePool>>,    
...
impl ResourceHandler<InferencePool> for InferencePoolResourceHandler<InferencePool> {
    ...
    async fn on_spec_not_changed(&self, resource_key: ResourceKey, resource: &Arc<InferencePool>, state: &State) -> Result<Action> {
        let () = state.save_inference_pool(resource_key, resource).expect("We expect the lock to work");
        Err(ControllerError::AlreadyAdded)
    }

    async fn on_new(&self, resource_key: ResourceKey, resource: &Arc<InferencePool>, state: &State) -> Result<Action> {
        let () = state.save_inference_pool(resource_key.clone(), resource).expect("We expect the lock to work");
    ...
    async fn on_status_not_changed(&self, resource_key: ResourceKey, resource: &Arc<InferencePool>, state: &State) -> Result<Action> {
...
impl InferencePoolResourceHandler<InferencePool> {
    async fn on_deleted(&self, id: ResourceKey, kube_gateway: &Arc<InferencePool>, state: &State) -> Result<Action> {
