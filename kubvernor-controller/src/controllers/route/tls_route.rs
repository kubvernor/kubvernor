...
use gateway_api_with_extensions::{common::RouteStatus, tlsroutes::TLSRoute};
use kube::{
    Api, Client, Resource,
...
impl TlsRouteController {
    ...
    async fn reconcile_grpc_route(resource: Arc<TLSRoute>, ctx: Arc<TlsRouteControllerContext>) -> Result<Action> {
        let controller_name = ctx.controller_name.clone();
        let grpc_route_patcher = ctx.tls_route_patcher.clone();
...
impl ResourceHandler<TLSRoute> for TlsRouteHandler<TLSRoute> {
    ...
    async fn on_new(&self, id: ResourceKey, resource: &Arc<TLSRoute>, state: &State) -> Result<Action> {
    ...
    async fn on_spec_changed(&self, id: ResourceKey, resource: &Arc<TLSRoute>, state: &State) -> Result<Action> {
        self.on_new_or_changed(id, resource, state).await
    }
...
impl TlsRouteHandler<TLSRoute> {
    async fn on_new_or_changed(&self, route_key: ResourceKey, resource: &Arc<TLSRoute>, _state: &State) -> Result<Action> {
    ...
    async fn on_deleted(&self, route_key: ResourceKey, resource: &Arc<TLSRoute>, _: &State) -> Result<Action> {
        let _ = Route::try_from(&**resource)?;
        ...
        self.common_handler.on_deleted(RouteType::Tls, route_key, parent_gateway_refs).await
    }
}
